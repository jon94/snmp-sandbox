...
with open(docker_compose_file, "a") as compose_file:
  ...
  container-{ips_with_dashes}:
    container_name: snmpsand-{ips_with_dashes}-container
...
with open(docker_compose_file, "a") as compose_file:
    for host in range(12, 14):
        compose_file.write(f'''
  container-172-1-1-{host}:
      container_name: snmpsand-172.1.1.{host}-container
      image: bhartford419/snmp_container:latest
      environment:
        - DD_TAGS=snmp_container:172.1.1.{host},auto-discovery:auto-discovered-device
      ports:
        - "161"
      volumes:
        - ./data/:/usr/local/share/snmpsim/data
...
# Append container info to docker-compose.yaml file for each IP address
    #Usable Host IP Range: 172.1.1.1 - 172.1.1.14
    compose_file.write(f'''
